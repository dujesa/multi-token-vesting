...
use pinocchio::{ProgramResult, account_info::AccountInfo, instruction::Seed, program_error::ProgramError};
use pinocchio_token::{instructions::Transfer, state::TokenAccount};

use crate::{AssociatedTokenAccount, Discriminator, MintAccount, VestedParticipant, PinocchioError, ProgramAccount, Schedule, SignerAccount};
...
impl<'a> TryFrom<&'a [AccountInfo]> for AddParticipantAccounts<'a> {
    ...
    fn try_from(accounts: &'a [AccountInfo]) -> Result<Self, Self::Error> {
        ...
        Ok(Self { authority, authority_ata, vault, vested_participant, participant_wallet, schedule, mint, system_program, token_program })
    }
}
#[repr(C, packed)]
...
impl<'a> TryFrom<&'a [u8]> for AddParticipantInstructionData {
    ...
    fn try_from(data: &'a [u8]) -> Result<Self, Self::Error> {
        ...
        Ok(Self { token_allocation_amount })
    }
}
...
impl<'a> AddParticipant<'a> {
    ...
    pub fn process(&mut self) -> ProgramResult {
        ...
        let mut vested_participant_state = VestedParticipant::load_mut(self.accounts.vested_participant)?;
